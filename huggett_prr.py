...
class state:
    ...
    def aggregate(self, vmu):
        ...
        spr = (1-sp)/sp
        mu = [array(vmu[t]).reshape(mls,zN,aN) for t in range(T)]
...
class cohort:
    ...
    def optimalpolicy(self, prices):
        ...
        self.vmu = self.mu.reshape(mls*zN*aN)
...
def transition_sub1(t,mu,prices,mu_t):
    T = prices.shape[1]
    c = cohort()
    mls = c.mls
...
def transition(N=20, TP=320, ng_i=1.012, ng_t=1.0):
    k_t, c_t = findsteadystate(ng=ng_t)
    k_i, c_i = findsteadystate(ng=ng_i)
    ...
    for n in range(N):
        ...
        for t, mu in enumerate(mu_tp):
            ...
            p = Process(target=transition_sub1, args=(t,mu,k_tp.prices,c_t.vmu))
            p.start()
            jobs.append(p)
        ...
        # print 'mu:',sum(mu_tp[0]), sum(mu_tp[5]), sum(mu_tp[10])
