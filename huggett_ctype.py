...
class state:
    ...
    def aggregate(self, c_tp):
        ...
        mu = [array(c_tp[t].vmu).reshape(mls,zN,aN) for t in range(len(c_tp))]
        self.K1 = zeros(T)
        self.Bq1 = zeros(T)
...
class cohort:
    ...
    def optimalpolicy(self, prices):
        ...
        mu = mu.reshape(mls*zN*aN)
        for i in range(mls*zN*aN):
            self.vmu[i] = mu[i]
...
def transition_sub1(c,prices,c_t,params):
    T = params.T
    mls = params.mls
    if c.y < T-1:
...
def transition(N=20, TP=180, ng_i=1.012, ng_t=1.0-0.012):
    ...
    c_tp = [cohort(params_tp,y=t) for t in range(TP)]
    for n in range(N):
        start_time = datetime.now()
        ...
        for c in c_tp:
            ...
            p = Process(target=transition_sub1, args=(c,k_tp.prices,c_t,params_tp))
            p.start()
            jobs.append(p)
